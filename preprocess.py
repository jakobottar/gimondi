...
def Preprocess(
    ...
    for i in tbar:
        ...
        if mask_path:
            new_mask_path = os.path.join(train_file_location, "masks")
        ...
        for p in range(num_patches):
            ...
            if mask_path:
                msk_filt = boundaryFilt(msk_crop)

            # generate filenames for image and mask
            img_name = os.path.join(new_image_path, f"{prefix}_{i}_{p}.png")
            if mask_path:
                mask_name = os.path.join(new_mask_path, f"{prefix}_{i}_{p}_mask.gif")

            if mask_path:
                csvwriter.writerow([img_name, mask_name])
            else:
                csvwriter.writerow([img_name, "None"])
